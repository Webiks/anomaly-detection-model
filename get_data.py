...
def download_json(path, index, last_minutes, host, port, user, password):
    ...
    time_query = {
        "@timestamp": {
            "gte": "now-{0}m".format(last_minutes+5000),
            "lt": "now-5000m"
        }
    }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', '-o', help='JSON output path', type=str, default="C:/Users/Tsabar/projects/monitor/json/test12.json", required=False)
    parser.add_argument('--index', '-i', help="ElasticSearch index", type=str, default='metricbeat-*', required=False)
    parser.add_argument('--lastMinutes', '-m', help="Last minutes to get data for", dest='last_minutes', type=int, default=3000, required=False)
    parser.add_argument('--host', '-a', help="ElasticSearch Host's ip/address", type=str, default='localhost', required=False)
